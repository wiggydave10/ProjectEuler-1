...
namespace Solutions.Problem055
{
    /*
        Lychrel numbers
        Problem 55
    ...
    */
    ...
    {
        public static bool IsLychrel(string n, int tries)
        {
            while (tries-- > 0)
            {
                var nReversed = new string(n.Reverse().ToArray());
                n = (BigInteger.Parse(n) + BigInteger.Parse(nReversed)).ToString();

                if (n.IsPalendrome()) return false;
            }
